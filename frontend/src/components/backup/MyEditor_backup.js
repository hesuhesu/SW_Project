...
import EditorToolBar, {insertHeart} from "../components/EditorToolBar";
...
// Undo and redo functions for Custom Toolbar
    ...
    loader.load(
      ...
      (gltf) => {
        if (gltf.scene) {
          ...
          console.log('모델의 모든 메쉬:', meshArray);
          */
          // const jsonString = JSON.stringify(gltf.scene.toJSON(), null, 2);
  ...
  function insert3DButton(){
    ...
    input.addEventListener('change', async () => {
      ...
      if (threeD !== ''){
        await axios.delete('http://localhost:5000/gltf_delete', {
          params: { threeD: threeD }
        }).then((response) => {})
      ...
      try {
        ...
        setThreeD(result.data.realName);
